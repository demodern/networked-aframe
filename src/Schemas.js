...
class Schemas {
  ...
  validateTemplate(schema, el) {
    if (!this.isTemplateTag(el)) {
      NAF.log.error(`Template for ${schema.template} is not a <template> or <a-template> tag. Instead found: ${el.tagName}`);
      return false;
  ...
  isTemplateTag(el) {
    return el.tagName.toLowerCase() === 'template' || el.tagName.toLowerCase() === 'a-template';
  }
